 ...
 *
 * IMPORTANT (the real cause of "API blocked"): the deployed backend must
 * answer cross-origin requests. If its ALLOWED_ORIGINS env var is stale or
 * malformed (e.g. pasted with the /ULPIN-GENERATION-PROTOTYPE subpath), the
 * browser silently rejects every response with "Failed to fetch" — even in a
 * private window, because that is CORS, not an extension. The backend now
 * ships a permissive CORS fallback, and opening /app/ skips CORS entirely,
 * so this should never appear again.
 */
